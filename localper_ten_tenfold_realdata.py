...
def ten_ten_fold_classification(datasetName):
    Dir = './dexPer_computed/'
    ...
    for i in range(10):
        print(f"-----  Split: {i+1} -----")
        array_indices = np.arange(num_networks)
        np.random.shuffle(array_indices)
        length = num_networks // 10

        te = []
        tr = []

        for k in range(10):
            print(f"-----  Split: {i+1} ----- Fold {k+1} -----")
            ie = array_indices[np.arange(start=k * length, stop=(k + 1) * length)]
            ir = np.setdiff1d(array_indices, ie)

            train_labels = labels[ir]
            test_labels = labels[ie]
            ...
            print(f"Train accuracy of Fold {k+1} = {sr}, test accuracy = {se}")
            tr.append(sr)
            te.append(se)
        
        print(f"Average train accuracy of Split {i+1} = {np.array(tr).mean()}, average test accuracy of Split {i+1} = {np.array(te).mean()}")
        train_accuracy.append(np.array(tr).mean())
        test_accuracy.append(np.array(te).mean())

    train_accuracy = np.array(train_accuracy)
    test_accuracy = np.array(test_accuracy)
    print(f"Average train accuracy = {train_accuracy.mean()}, average test accuracy = {test_accuracy.mean()}, test sd = {np.std(test_accuracy)}")

def main(dataset_name):
    ten_ten_fold_classification(dataset_name)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Ten ten-fold graph classification for TUDataset')
