...
def single_ten_fold_classification(datasetName, k_ring, b0):
    Dir = './dexPer_computed/'
    ...
    k_ring = k_ring
    vec_num_clusters_dexPer0 = b0
    vec_num_clusters_dexPer1 = b0
    n_estimators = 400
    max_depth = 10
    ...
    if not os.path.exists(vec_dexPers_filename):
        ...
        for i in range(num_networks):
            ...
            for idx, item in enumerate(dexPer0_Each[i]):
                ...
                if len(eh) == 0:
                    dexPer0_pairs[idx] = [[1.0, 1.0]]
                    dexPer0_weights[idx] = [1]
                else:
            ...
            for idx, item in enumerate(dexPer1_Each[i]):
                ...
                for key, value in item.items():
                    ...
                    eh_weights.append(value)
                if len(eh) == 0:
                    dexPer1_pairs[idx] = [[1.0, 1.0]]
                    dexPer1_weights[idx] = [1]
                else:
    ...
    for (ir, ie) in folds:
        ...
        vec_ATOL_dexPer0 = Atol(quantiser=MiniBatchKMeans(n_clusters=vec_num_clusters_dexPer0, random_state=42), weighting_method="iidproba")
        ...
        vec_ATOL_dexPer1 = Atol(quantiser=MiniBatchKMeans(n_clusters=vec_num_clusters_dexPer1, random_state=42), weighting_method="iidproba")
    ...
    print(f"k_ring = {k_ring}, b_0 = {b0}, Average test accuracy = {(np.mean(te))}")
    return np.mean(te)
...
def main(dataset_name):
    ...
    for k in [1,2,3,4]:
        j = 0
        for b0 in [10, 20, 30, 40, 50, 60, 70, 80]:
            result_matrix[i][j] = single_ten_fold_classification(dataset_name, k, b0)
            j += 1

        t = np.arange(10, 90, 10)
        plt.xlabel('budget')
        plt.ylabel('test accuracy')
        plt.title(f'k_ring = {k}')
        plt.plot(t, result_matrix[i][:], 'bs-')
        plt.ylim(0.4, 1)
        plt.show()
        i += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--dataset_name', type=str, default="MUTAG", help="Default is 'MUTAG'. Check https://chrsmrrs.github.io/datasets/docs/datasets/ for all the available datasets.")
